...
public static partial class CipherExtensions
{
	...
	public static ICipher Pin<TKey>(this IKeyedCipher<TKey> keyedCipher, TKey key) => new PinnedKeyCipher<TKey>(keyedCipher, key);

	public static IKeyedCipher<string> AsStringKeyed(this IKeyedCipher<char[]> cipher) => new CharArrayToStringKeyedCipher(cipher);
	...
	private sealed class CharArrayToStringKeyedCipher : IKeyedCipher<string>
	{
		public CharArrayToStringKeyedCipher(IKeyedCipher<char[]> inner)
		{
			_inner = inner;
		}

		private readonly IKeyedCipher<char[]> _inner;

		public void Encrypt(ReadOnlySpan<char> plaintext, Span<char> ciphertext, string key, out int written) =>
			_inner.Encrypt(plaintext, ciphertext, key.ToCharArray(), out written);

		public void Decrypt(ReadOnlySpan<char> ciphertext, Span<char> plaintext, string key, out int written) =>
			_inner.Decrypt(ciphertext, plaintext, key.ToCharArray(), out written);
	}
}
